...
void initialize_python_once() {
    ...
    if (!initialized) {
        Py_Initialize();
        PyRun_SimpleString("import sys; sys.path.insert(0, '/afs/cern.ch/user/u/urahaman/Manuel/NOvA_data/NH')");
...
int main(int argc, char *argv[]) {
  double M_PI=3.14; 
    ...
    int i=atoi(str);
     ...
     /* Intitialize output */
  FILE *outfile = NULL;
  sprintf(MYFILE, "/eos/user/u/urahaman/SWAN_projects/long-baseline/globes/Manuel/NOvA_data/NH/chisq_NHtest_NOvA_nu+anu_app+disapp_2024_newphysics_%d.dat", i);
  ...
  if (outfile == NULL)
  {
   ...
   glbSetOscParams(test_values, thedcp, GLB_DELTA_CP);
    ...
    double a = 0.031e-3, s=0.00066; /*Errors in DM31 and th13*/
        for (l = 0.43; l <= 0.611; l += 0.01) {
            for (x = 0.02237-3*s; x < 0.02237+3.01*s; x = x + s) {
                for (k = 2.528e-3 - 3*a; k < 2.528e-3 +3.01*a; k = k + a) {
